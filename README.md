...
`数据获取时间`（星球）和 `时间`（快讯）都格式化成 **北京时间 `YYYY-MM-DD HH:MM:SS`**：

| 上游（UTC，7 位小数秒） | 输出 |
|---|---|
| `2026-09-14T03:16:56.8288321Z` | `2026-09-14 11:16:56` |
| `2026-09-13T17:00:00Z` | `2026-09-14 01:00:00` |

用固定 `+08:00` 偏移而不是 `zoneinfo("Asia/Shanghai")`：中国自 1991 年起不再实行夏令时，
两者结果一致，但固定偏移省掉了 Windows 上可能缺失的 IANA 时区库依赖。
解析不了的时间戳原样返回，不编造。

**注意**：JSON 模式（默认）仍然返回原始 UTC ISO 字符串，只有文本模式做这个转换。

### 与文档示例的四处差异（实现时的判断）

1. **时间格式化成北京时间、精确到秒**。文档示例里是上游原样的
   `2026-09-14T01:31:44.2973993Z`（UTC + 7 位小数秒），按要求改成 `2026-09-14 09:31:44`。
2. **没有输出 `// planet.name` 这类行尾注释。** 它们是文档作者标注的**字段来源**，不是输出内容。
   ...
   —— 说明注释是凭记忆手写的。
3. **`数据获取时间` 跟示例值走**：优先用实测窗口的结束时刻（`liberation_rate.measured.to`，
   那才是「数据是什么时候的」），没有实测时才退回 `generated_at`。
4. **`信息` 里的换行是真实换行**，不是字面量 `\n`。JSON 里 `"a\n\nb"` 本来就是真换行，
   输出成多行更易读。
